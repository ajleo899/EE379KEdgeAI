...
#for batch in batch_sizes:
        ...
        print('OPTIMIZER: {opt}'.format(opt=opt))
        ...
        losses_opt.append(epoch_losses)


    a = plt.figure(1)
    for i in range(len(losses_opt)):
        plt.plot(losses_opt[i])
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.title('Training Loss for Optimizer: {opt}'.format(opt = opt))
    plt.grid(True)
    plt.legend(["0.01", "0.001", "0.0001"], loc ="upper right")
    a.savefig('simpleCNN_loss_plots_{optimizer}.png'.format(optimizer = opt))
    plt.cla()
